...
from models.dual_net import dual_UNet
...
parser.add_argument('--model', type=str, default='dual_unet', choices=['unet', 'unet_esa', 'our', 'unet_border','dual_unet'])
...
if args.model == 'unet':
    ...
    net =unet_fpn.UNet_fpn(num_classes=dataset.NUM_CLASSES,in_channels=dataset.CHANNELS_NUM, is_esa=True ,is_aspp=False)
elif args.model == 'unet_border':
    net = UNet_border(num_classes=dataset.NUM_CLASSES,in_channels=dataset.CHANNELS_NUM,is_esa=True)
elif args.model == 'dual_unet':
    net = dual_UNet(num_classes=dataset.NUM_CLASSES,in_channels=dataset.CHANNELS_NUM)
...
for epoch in range(start_epoch, args.epoch):
    ...
    for index, data in enumerate(train_loader):
        ...
        D1 = torch.tensor(border(labels.numpy())).float()

        optimizer.zero_grad()
        outputs,output2 = net(inputs)   # 如果是我们改进的模糊边界unet模型
        ...
        if isinstance(outputs, list):
            ...
            loss = criterion1(outputs, labels.long()) + criterion2(outputs, labels.long()) + criterion1(output2,D1.long()) + criterion2(output2, labels.long())
            # 计算在该批次上的平均损失函数
        loss /= inputs.size(0)
    ...
    with torch.no_grad():
        for _, data in enumerate(val_loader):
            ...
            D4 = down(D3)
            D5 = down(D4)
            outputs,M1,M2,M3,M4,M5,M6,M7,M8,M9 = net(inputs)
            ...
            if isinstance(outputs, list):
                ...
                loss = criterion1(outputs, labels.long()) + criterion2(outputs, labels.long()) + criterion1(M1,D1.long()) + criterion1(M2,D2.long()) + criterion1(M3,D3.long()) + criterion1(M4,D4.long()) + criterion1(M5,D5.long()) + criterion1(M6,D4.long()) + criterion1(M7,D3.long()) + criterion1(M8,D2.long()) + criterion1(M9,D1.long())
            # 计算在该批次上的平均损失函数
            loss /= inputs.size(0)
