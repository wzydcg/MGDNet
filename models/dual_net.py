...
from models.SA import ImageMultiHeadAttention
...
class dual_UNet2(nn.Module):
    ...
    def __init__(self, num_classes, in_channels):
        ...
        self.pred1 = nn.Sequential(
            nn.Conv2d(nb_filters[3], nb_filters[3], kernel_size=3, padding=1, stride=1),
            nn.BatchNorm2d(nb_filters[3]),
            nn.ReLU(inplace=True),
            nn.Conv2d(nb_filters[3], nb_filters[3] * 2, kernel_size=3, padding=1, stride=1)
        )
        self.sa = ImageMultiHeadAttention(embed_dim=512, num_heads=8)
        self.sa1 = ImageMultiHeadAttention(embed_dim=256, num_heads=8)
        # 最后接一个Conv计算在所有类别上的分数
        self.final = nn.Conv2d(nb_filters[0], num_classes, kernel_size=1, stride=1)
    ...
    def forward(self, input):
        ...
        k1_1, v1_1 = self.pred1(x3_1).chunk(2, dim=1)
        k2_1, v2_1 = self.pred1(y3_1).chunk(2, dim=1)

        A1_1 = self.sa1(x3_1, k1_1, v2_1)
        A2_1 = self.sa1(x3_1, k2_1, v1_1)
        A3_1 = self.sa1(x3_1, k1_1, v1_1)
        B1_1 = self.sa1(y3_1, k1_1, v2_1)
        B2_1 = self.sa1(y3_1, k2_1, v1_1)
        B3_1 = self.sa1(y3_1, k2_1, v2_1)
        x3_1_1 = self.sa1(A3_1, A1_1, A2_1)
        y3_1_1 = self.sa1(B3_1, B1_1, B2_1)

        x2_2 = self.conv2_2(torch.cat([x2_0, self.up(x3_1_1)], dim=1))
        y2_2 = self.conv2_2(torch.cat([x2_0, self.up(y3_1_1)], dim=1))
        x1_3 = self.conv1_3(torch.cat([x1_0, self.up(x2_2)], dim=1))
        y1_3 = self.conv1_3(torch.cat([x1_0, self.up(y2_2)], dim=1))
        x0_4 = self.conv0_4(torch.cat([x0_0, self.up(x1_3)], dim=1))
        y0_4 = self.conv0_4(torch.cat([x0_0, self.up(y1_3)], dim=1))
...
if __name__ == '__main__':
    ...
    net=dual_UNet2(num_classes=2, in_channels=3)
    print(net(x)[0].shape)
    print(net(x)[1].shape)
