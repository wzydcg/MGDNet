...
from acmix.acmix import ACmix
from models.fft_tensor import process_image_with_equations_tensor
from models.TXD import CompleteConditionedDiffusionPipeline
...
class UNet_fpn(nn.Module):
    def __init__(self, num_classes, in_channels, is_esa=True, is_aspp=False, categories=6):
        super().__init__()
        nb_filters = [32, 64, 128, 256, 512]  # 列出卷积核的尺寸
        self.is_aspp = is_aspp
        self.is_esa = is_esa
        ...
        self.depth_conv = nn.Conv2d(in_channels=32, out_channels=1, kernel_size=1)
        self.conv_class = nn.Conv2d(in_channels=256, out_channels=6, kernel_size=1)

        self.condTXD = CompleteConditionedDiffusionPipeline(latent_dim=24, window_size=7, num_classes=6) # 超参数随时准备修改
        # 分类器
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(32 * 2, categories)
        # 标准的2倍上采样和下采样，因为没有可以学习的参数，可以共享
        self.down = nn.MaxPool2d(kernel_size=2, stride=2)
        self.up = nn.UpsamplingBilinear2d(scale_factor=2)
        self.acmix1 = ACmix(in_planes = 32, out_planes = 32, h = 224)
    ...
    def forward(self, input):
        # 获取纹理
        _, tex = process_image_with_equations_tensor(input, alpha=10)
        # 下采样编码
        ...
        if self.is_esa:
            f8_0 = f8_0 + self.acmix1(f8_0)

        # 特征融合并进行上采样解码，使用concatenate进行特征融合
        x5_0 = self.conv3_3(torch.cat([self.conv2(f11_0), self.up(x4_0)], dim=1))
        x5_class = self.conv_class(x5_0)

        class_labels = []
        for i in range(4):
            # 计算每个类别在掩码中的像素数量
            mask = x5_class[i]
            class_pixel_counts = mask.sum(dim=(1, 2))

            # 选择像素最多的类别作为主类别
            pred_class = class_pixel_counts.argmax().item()
            class_labels.append(pred_class)
        # 将列表转换为tensor
        class_labels_tensor = torch.tensor(class_labels, device=x5_class.device)
        ...
        _ ,_ ,_ ,_ , txd_enhanced_feature = self.condTXD(self.depth_conv(f5_0), tex3, self.depth_conv(f11_0), class_labels_tensor, x3_0)
        pooled = self.global_avg_pool(txd_enhanced_feature)  # [B, C, 1, 1]
        classifier = self.fc(pooled.view(pooled.size(0), -1))  # [B, C] -> [B, num_classes]
        return output, classifier, txd_enhanced_feature, class_labels_tensor

if __name__ == '__main__':
    x=torch.rand([4, 3, 224, 224])
    net=UNet_fpn(num_classes=3, in_channels=3)
    print(net(x)[0].shape)
    print(net(x)[1].shape)
